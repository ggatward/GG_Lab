...
network --bootproto static --ip=172.22.4.7 --netmask=255.255.255.0 --gateway=172.22.4.1 --nameserver=172.22.1.3,172.22.1.5 --mtu=9000 --hostname ovirt-engine.lab.home.gatwards.org
...
echo logvol /home --fstype xfs --name=lv_home --vgname=vg_sys --size=1024 --fsoptions="noatime,nosuid,nodev" >> /tmp/diskpart.cfg
echo logvol /tmp --fstype xfs --name=lv_tmp --vgname=vg_sys --size=8192 --fsoptions="noatime,nosuid,nodev" >> /tmp/diskpart.cfg
echo logvol /var --fstype xfs --name=lv_var --vgname=vg_sys --size=10240--fsoptions="noatime,nosuid,nodev" >> /tmp/diskpart.cfg
...
%post
logger "Starting anaconda ovirt-engine.lab.home.gatwards.org postinstall"
exec < /dev/tty3 > /dev/tty3
#changing to VT 3 so that we can see whats going on....
/usr/bin/chvt 3
(
